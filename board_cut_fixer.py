...
#        cv2.waitKey(0)
    ...
    #        print "down angle=" + str(down_angle)
            ...
            if(not self.is_final_proj_correct(proim)):
                print("CUT BOARD FIXER HAS FUCKING FAILED")
                return None
            return proim
        except:
    ...
    def do_conv_ver_right(self, img):
        # cv2.imshow("1", img)
        # cv2.waitKey(0)
        window_length = len(img) // 25
        window_height = len(img[0]) // 16

        changeCounter = 0
        lastcolorFlag = 0  ##zero if last is white, 1 if black
        newim = copy.deepcopy(img)
        for i in range(0, len(img) - window_height - 1, 10):
            whitepix = 0
            blackpix = 0
            for j in range(i, i + window_height):
                for k in range(len(img[0]) - window_length, len(img[0])):
                    if k >= len(newim[0]) or j >= len(newim):
                        break
                    #         newim[j][k] = 0
                    if (newim[j][k] > 0):
                        whitepix = whitepix + 1
    ...
    def do_conv_ver_left(self, img):
        ...
        for i in range(0, len(img) - window_height - 1, 10):
            ...
            blackpix = 0
            for j in range(i, i + window_height):
                for k in range(0, window_length):
                    if k >= len(newim[0]) or j >= len(newim):
                        break
                    if (newim[j][k] > 0):
                        whitepix = whitepix + 1
            ...
            if whitepix > blackpix:
                ...
                specificcolorFlag = 0
            #            print(specificcolorFlag)
            if specificcolorFlag != lastcolorFlag:
                changeCounter = changeCounter + 1
                lastcolorFlag = specificcolorFlag
        return changeCounter

    def gausThresholdChess2(self, img, max=GAUSS_MAX_VALUE,
                            size=GAUSS_BLOCK_SIZE, c=GAUSS_C):
        gaus = cv2.adaptiveThreshold(img, max,
                                     cv2.THRESH_BINARY,
                                     cv2.THRESH_BINARY, size,
                                     c)
    ...
    def is_final_proj_correct(self, img):
        img = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
        img = self.gausThresholdChess2(img, 255, 195, 1)

        if self.do_conv_ver_left(img) > 4 and self.do_conv_ver_right(img) > 4:
            return True
        return False
