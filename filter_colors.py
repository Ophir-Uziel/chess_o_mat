...
BLACK = (0.0, 0.0, 0.0)
MINIMAL_PLAYER_BOARD_RATIO = 0.1
PIXELS_FOR_MAIN_COLORS = (600, 600)
PIXELS_FOR_SIDE = (160,160)
PIXELS_SQUARE = (20, 20)
BLACK_NUM = 1
WHITE_NUM = 2
UP = True
...
class filter_colors:
    ...
    def color_dist(self, color1, color2):
        return abs(color1[2] - color2[2] - color2[0] + color1[0])
    ...
    def rank_side(self, im1, im2, is_right):
        main_colors = self.main_colors
        im1 = cv2.resize(im1, PIXELS_FOR_SIDE)
        im2 = cv2.resize(im2, PIXELS_FOR_SIDE)
        board_sz = len(im1)
        sq_sz = len(im1[0]) // 8
        if is_right:
            im_1 = im1[0:board_sz, 0:board_sz]
            im_2 = im2[0:board_sz, 0:board_sz]
        else:
            im_1 = im1[0:board_sz, 0:int(0.25*sq_sz)]
            im_2 = im2[0:board_sz, 0:int(0.25*sq_sz)]
        cat_im_1 = self.fit_colors_side(im_1, main_colors)
        cat_im_2 = self.fit_colors_side(im_2, main_colors)
        rank = self.find_side_rank(cat_im_1, cat_im_2)
        num_of_pix = (board_sz * sq_sz) / 8
        return rank / num_of_pix


    def find_side_rank(self, im1, im2):
        rank = 0
        for rowidx in range(len(im1)):
            for pixidx in range(len(im1[0])):
                if im2[rowidx][pixidx] - im1[rowidx][pixidx] != 0:
                    rank += 1
        return rank

    def fit_colors_side(self, im, main_colors):
        new_im = []
        for rowidx in range(len(im)):
            i = 0
            row = im[rowidx]
            ...
            for pix in row:
                ...
                if dist < min_dist:
                    min_dist = dist
                    new_im[rowidx][i] = BLACK_NUM
                dist = self.color_dist(pix, main_colors[2])
                if dist < min_dist:
                    min_dist = dist
                    new_im[rowidx][i] = self.ME_NUM
                dist = self.color_dist(pix, main_colors[3])
                if dist < min_dist:
                    new_im[rowidx][i] = self.HIM_NUM
                i += 1
        return new_im
