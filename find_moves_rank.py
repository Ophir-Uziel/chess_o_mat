...
class find_moves_rank:
    ...
    def get_move(self,sources_place, sources_self, sources_above,
        ...
        if angle_dir is not None:
            for idx in self.mistake_idxes:
                misc.imsave(angle_dir + sources_place[idx] + '.jpg', sources_self[idx])
                misc.imsave(angle_dir + sources_place[idx] + '_abv.jpg', sources_above[idx])
        ...
        if angle_dir is not None:
            for idx in self.mistake_idxes:
                misc.imsave(angle_dir + targets_place[idx] + '.jpg', targets_self[idx])
                misc.imsave(angle_dir + targets_place[idx] + '_abv.jpg',
                            targets_above[idx])
    ...
    def best_target_for_given_source(self, source_place, targets_place,
        ...
        for i in range(len(matches)):
            if target_img_dict[matches[i]] > best_match_rank:
                best_idx = i
                best_match_rank = target_img_dict[matches[i]]
        return matches[best_idx], best_match_rank
    ...
    '''
    def check_vector(self, vector, max1, min1):
        ...
        return min1 < w_max < max1, w_max  #T/F, longest sequence
    ...
    def check_square_below(self, img):
        ...
        misc.imsave('0crop_im.jpg', crop_img)
        row_len = len(crop_img[0])
        d_vector = crop_img[len(crop_img) - 1]

        d_b, d_len = self.check_vector(d_vector[int(row_len*SIDE_CROP_RT):int(row_len-(row_len*SIDE_CROP_RT))], row_len, MIN_WHITE_RT*row_len)
        if not d_b:
            return 0
        max1 = d_len * MAX_RT
        min1 = d_len/MIN_RT
        w_rank = 4*d_len/row_len
        #metric_ratio = 2*d_len/row_len

        for i in range(len(crop_img)):
            v_b, v_len = self.check_vector(crop_img[len(crop_img) - i - 1][int(row_len*SIDE_CROP_RT):int(row_len-(row_len*SIDE_CROP_RT))], max1, min1)
            #if not v_b:
             #   if true_counter > 1:
              #      return 0
               # true_counter = 0
    ...
    def check_square(self,imgOneZero):
        crop_img = []
        imgOneZero = imgOneZero[int(len(imgOneZero)*SELF_CROP_RT):int(len(imgOneZero)*(1-SELF_CROP_RT))]
        for row in imgOneZero:
            crop_img.append(row[(int(len(row)*SELF_CROP_RT)):int(len(row)*(1-SELF_CROP_RT))])

        WXcm, WYcm = self.centerMass(crop_img, WHITE)
        BXcm, BYcm = self.centerMass(crop_img, BLACK)
        yahas = self.meanDist(crop_img, WHITE, WXcm, WYcm) / self.meanDist(crop_img,
                                                                   BLACK, BXcm,
                                                                   BYcm)+0.00000001
